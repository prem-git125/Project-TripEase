...
const Home = () => {
	...
	useEffect(() => {
		...
		fetch("http://localhost:7000/api/countriescount")
		  .then(response => response.json())
		  .then(data => setCountryCount(data.totalCountry))
		  .catch(error => console.error('Error fetching country count:', error));
	  }, []);

	  useEffect(() => {
		// Fetch user count from backend API
		fetch("http://localhost:7000/api/statecount")
		  .then(response => response.json())
		  .then(data => setStateCount(data.totalState))
		  .catch(error => console.error('Error fetching state count:', error));
	  }, []);

	  useEffect(() => {
		// Fetch user count from backend API
		fetch("http://localhost:7000/api/citycount")
		  .then(response => response.json())
		  .then(data => setCityCount(data.totalCity))
		  .catch(error => console.error('Error fetching city count:', error));
	  }, []);

	  useEffect(() => {
		// Fetch user count from backend API
		fetch("http://localhost:7000/api/usercount")
		  .then(response => response.json())
		  .then(data => setUserCount(data.totalUser))
		  .catch(error => console.error('Error fetching user count:', error));
	  }, []);

	  useEffect(() => {
		// Fetch user count from backend API
		fetch("http://localhost:7000/api/guidercount")
		  .then(response => response.json())
		  .then(data => setGuiderCount(data.totalGuider))
		  .catch(error => console.error('Error fetching guider count:', error));
	},[]);
	...
	return (
		<>
			...
			<div className="intro-section bg-white" style={{margin:'30px',padding:'0px'}}>
				<div className="auto-container">
					...
					<div className="row clearfix justify-content-center">
						...
						<div className="intro-block col-lg-4 col-md-6 col-sm-12 wow fadeInUp" data-wow-delay="300ms"
							...
							<div className="inner-box">
								...
								<h4>Best Vlogs</h4>
								<p className="travilo-text">"Join me on a whirlwind journey through life's extraordinary moments! From the bustling streets of urban exploration to serene escapes in nature's embrace, my vlog captures the essence of adventure, inspiration, and authenticity."</p>
							</div>
						</div>
						...
						<div className="intro-block col-lg-4 col-md-6 col-sm-12 wow fadeInUp" data-wow-delay="600ms"
							...
							<div className="inner-box">
								...
								<p className="travilo-text">"Satisfaction is finding contentment in the journey, not just the destination. It's about embracing the present moment with gratitude and purpose. Whether achieving a long-awaited goal or cherishing simple pleasures."</p>
							</div>
						</div>
					</div>
				</div>
		...
		about 2 */}
			...
			<div className="about-two  bg-white" style={{margin:'70px',padding:'10px'}}>
								...
								<div className="inner wow fadeInRight" data-wow-duration="1500ms" data-wow-delay="0ms">
									<div className="title-box">
										...
										<div className="travilo-text">
											<p> We pride ourselves on our commitment to making your
												...
												<div className="inner-box clearfix">
													<div className="fact-count">
														<div className="count-box"><span className="count-text" data-stop="2"
															data-speed="1000">{countryCount}</span>
														</div>
													</div>
													<div className="fact-title">Total <br />Country</div>
												</div>
											</div>
											<div className="fact-block col-lg-4 col-md-4 col-sm-6">
												<div className="inner-box clearfix">
													<div className="fact-count">
														<div className="count-box"><span className="count-text" data-stop="5"
															data-speed="1500">{stateCount}</span>
														</div>
													</div>
													<div className="fact-title">Total <br />State</div>
												</div>
											</div>
											<div className="fact-block col-lg-4 col-md-4 col-sm-6">
												<div className="inner-box clearfix">
													<div className="fact-count">
														<div className="count-box"><span className="count-text" data-stop="8"
															data-speed="2000">{cityCount}</span>
														</div>
													</div>
													<div className="fact-title">Total <br />City</div>
												</div>
											</div>
										</div>
									</div>
			...
			<div className="why-us-two" style={{margin:'0px',padding:'10px'}}>
				<div className="auto-container">
					<div className="outer-box">
									...
									<div className="features">
										<div className="f-block-three">
											<div className="inner-box">
												...
												<h4>Amazing Tour</h4>
												<div className="travilo-text">The description effectively captures the essence of satisfaction in a concise and thoughtful manner.</div>
											</div>
										</div>
										<div className="f-block-three">
											<div className="inner-box">
												<div className="icon"><img src={icon8} alt="" /></div>
												<h4>Best Travel Guiders</h4>
								...
								<div className="inner wow fadeInRight" data-wow-duration="1500ms" data-wow-delay="0ms">
									...
									<div className="fact-block f-1">
										<div className="inner-box clearfix">
											<div className="fact-count">
												<div className="count-box"><span className="count-text" data-stop="5"
													data-speed="2000">{userCount}</span></div>
											</div>
											<div className="fact-title">Total <br />Users</div>
										</div>
									</div>
										...
										<div className="inner-box clearfix">
											<div className="fact-count">
												<div className="count-box"><span className="count-text" data-stop="20"
													data-speed="2000">{guiderCount}</span></div>
											</div>
											<div className="fact-title">Total <br />Guider</div>
										</div>
									</div>
								</div>
							</div>
								...
								<div className="text-col col-lg-9 col-md-12 col-sm-12">
									<div className="inner">
										<div className="title-box">
											...
											<h2><span>Get Help from Our Travel Guiders <i><img
												src={white} alt="" /></i></span></h2>
											<p className="travilo-text"> Whether leading a group through a city's landmarks, guiding a hike through nature reserves, or facilitating cultural immersion experiences, tour guides serve as ambassadors for the places they represent.</p>
										</div>
									</div>
								</div>
